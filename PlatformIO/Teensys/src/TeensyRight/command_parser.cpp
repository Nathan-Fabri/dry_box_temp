...
void parseIncomingData(String input, float& ambientTemp, float& shellTemp, int& humidity, int& shellTime, int& FanSpeed, bool& machineState, float& deltaShellTemperature, float& deltaAirTemperature, float& deltaHumidity, float& deltaShellFanAirFlow, float& currAmbientTemperature, float& currShellTemperature, float& currHumidity) {
    ...
    if (input.startsWith("ALLOFF")) {
        ...
        Serial.println("ACK-TeensyRight");
    }
    else if(input.startsWith("PID-")){
        input = input.substring(4);
        float Kp = getValue(input, "Kp").toFloat();
        float Ki = getValue(input, "Ki").toFloat();
        float Kd = getValue(input, "Kd").toFloat();
        ...
        SerialUSB1.println("UKN-TeensyRight");
    }
}