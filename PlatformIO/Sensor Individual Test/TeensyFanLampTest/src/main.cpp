...
bool lightsOn = false; // Keeps track of whether the lights are on
...
void loop() {
  ...
  if (Serial.available() > 0) {
    ...
    if (input == '0') {
      ...
      uint8_t dimmerLevel = lightsOn ? 0 : 100; // 0 = full brightness, 100 = off
      for (int addr = 0; addr < 4; addr++) {
        for (int ch = 0; ch < 4; ch++) {
          sendDimmerCommand(DIMMER_ADDRESSES[addr], CHANNELS[ch], dimmerLevel);
        }
      }
